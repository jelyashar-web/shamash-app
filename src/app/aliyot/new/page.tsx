...
export default function NewAliyahPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.date) {
      toast('יש לבחור תאריך', 'error');
      return;
    }

    setIsLoading(true);
    try {
      const res = await fetch('/api/aliyot', {
        method: 'POST',
      ...
      if (res.ok) {
        toast('עלייה נוספה בהצלחה', 'success');
        router.push('/aliyot');
      } else {
        const data = await res.json();
        toast(data.error || 'שגיאה בהוספת עלייה', 'error');
      }
    } catch {
      toast('שגיאה בהוספת עלייה', 'error');
    } finally {
      setIsLoading(false);
    }
  };
